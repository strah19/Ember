...
namespace Ember {
	void Camera::WorldToScreen(float world_x, float world_y, int& screen_x, int& screen_y) {
		screen_x = (int)((world_x - offset_x) * scale_x);
		screen_y = (int)((world_y - offset_y) * scale_y);
	}

	void Camera::ScreenToWorld(int screen_x, int screen_y, float& world_x, float& world_y) {
		world_x = ((float)screen_x / scale_x) + offset_x;
		world_y = ((float)screen_y / scale_y) + offset_y;
	}

	void Camera::Pan(Ember::ButtonIds button_id, Ember::Events* events) {
	...
	void Camera::Scale(Ember::Events* events, const Ember::Vec2& scalar) {
		float mouse_x = (float)events->MousePosition().x;
		float mouse_y = (float)events->MousePosition().y;
